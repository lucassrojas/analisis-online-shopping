...
select CustomerID, Gender, Location, Transaction_ID, Transaction_Date, Product_Category, Quantity, Avg_Price into file2 from dbo.[file]

select top 100 * from file2

alter table file2 alter column CustomerID varchar(250)
...
-- Top 4 Marcas por Revenue

with revenues as (select Product_Category as marca, sum(Avg_Price * Quantity) as revenue_marca from file2 group by Product_Category) select top 4 *, rank() over(order by revenue_marca desc) ranking from revenues


-- Top 4 Clientes por Revenue

with revenues as (select CustomerID, sum(Avg_Price * Quantity) as revenue_cliente from file2 group by CustomerID) select top 4 *, rank() over(order by revenue_cliente desc) ranking from revenues


-- Distribución de Revenue por Ciudad

select Location as ciudad, sum(avg_price * quantity) as revenue_ciudad from file2 group by Location

with revenues as (select Location as ciudad, sum(avg_price * quantity) as revenue_ciudad from file2 group by Location) select ciudad, revenue_ciudad * 100.0 / (select sum(avg_price * quantity) as revenue from file2) as porcentaje from revenues
